...
export async function detectProject(projectDir) {
    ...
    const rootIsMonorepo = Boolean(packageJson.workspaces) ||
    ...
    if (rootIsMonorepo && !rootIsFrontend && candidates.length > 1) {
        ...
        throw new Error(`Multiple frontend app candidates found. Re-run with --project pointing at one app:\n${list}`);
    }
    if (rootIsMonorepo && !rootIsFrontend && candidates.length === 1) {
        warnings.push(`Monorepo root detected; only one frontend app found at ${path.relative(root, candidates[0])}. Prefer passing --project explicitly when writing real assets.`);
    }
    return { root, packageJson, warnings };
}
async function findFrontendCandidates(root, maxDepth) {
    const candidates = [];
...
function isFrontendPackage(packageJson) {
    const deps = {
...
async function hasFrontendShape(dir) {
