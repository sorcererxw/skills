...
export async function generateIcons(options: GenerateOptions): Promise<WrittenFile[]> {
  ...
  if (config.favicon.enabled && config.favicon.svg) {
    await writeFiles(
      await resolveIconTargets(project.root, path.join(publicDir, "logo.svg"), [
        "public/logo.svg",
        "public/brand.svg",
        "public/logo-icon.svg"
      ], [path.join(publicDir, "logo.svg")]),
      () => makeThemeLogoSvg(svg, config.colors.lightIcon, config.colors.darkIcon),
      options.dryRun,
      written
    );
