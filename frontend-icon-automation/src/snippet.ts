...
export async function makeHeadSnippet(config: IconConfig): Promise<string> {
  const lines: string[] = [];
  ...
  lines.push(
    `<meta name="theme-color" content="${config.colors.themeColor}" media="(prefers-color-scheme: light)" />`,
    `<meta name="theme-color" content="${config.colors.themeColorDark}" media="(prefers-color-scheme: dark)" />`
  );
